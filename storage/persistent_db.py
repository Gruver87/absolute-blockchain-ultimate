...
class PersistentDB:
    """Persistent storage (LevelDB-style)"""

    def __init__(self, path: str = "chain_data"):
        self.path = path
    ...
    def put_state(self, root: str, state_data: Dict):
        with self._lock:
            self._state[root] = state_data
            self._save_store("state.json", self._state)

    def get_state(self, root: str) -> Optional[Dict]:
