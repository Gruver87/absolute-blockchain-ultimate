# consensus/engine_v42.py
"""
Consensus Engine v42 — LMD-GHOST + Finality (Casper-style)
"""

from typing import Dict, List, Optional, Any
from consensus.lmd import LMDTable
from consensus.ghost import select_head, get_cumulative_weight
from consensus.finality import FinalityEngine
from consensus.epoch import EpochManager


class ConsensusEngineV42:
    """
    Full Ethereum-style consensus:
    - LMD table for votes
    - GHOST for fork choice
    ...
    def on_attestation(self, validator_id: str, block_hash: str, slot: int):
        ...
        # Update finality weights
        block = self._blocks.get(block_hash)
        if block:
            epoch = self.epoch_mgr.get_epoch(block.get("number", 0))
            weights = self.lmd.get_weights()
            self.finality.add_vote(epoch, block_hash, weights.get(block_hash, 0))
    ...
    def finalize_epoch(self, epoch: int) -> dict:
        """Try to finalize an epoch"""
        return self.finality.update(epoch)

    def is_finalized(self, block_hash: str) -> bool:
        """Check if block is in finalized epoch"""
        block = self._blocks.get(block_hash)
        if not block:
            return False
        epoch = self.epoch_mgr.get_epoch(block.get("number", 0))
        return epoch in self.finality.get_finalized_epochs()
