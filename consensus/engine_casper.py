...
"""
Consensus Engine v43 — LMD-GHOST + Casper FFG Finality
"""
...
class ConsensusEngineCasper:
    """
    Full Ethereum-style consensus with correct Casper FFG:
    - LMD table for votes
    - GHOST for fork choice
    - Casper FFG for finality (two-step rule)
    """

    def __init__(self, epoch_size: int = 3):
        self.lmd = LMDTable()
        self.finality = CasperFinality()
    ...
    def is_finalized(self, block_hash: str) -> bool:
        ...
        if not block:
            return False
        return self.finality.is_finalized(block.get("number", 0), self.epoch_mgr)
