...
# Полное тестирование JWT аутентификации
...
def test_auth():
    print("=" * 60)
    print("Полное тестирование JWT аутентификации")
    print("=" * 60)
    ...
    resp = requests.post(f"{BASE_URL}/api/auth/login",
                         json={
                             "address": "foundation",
                             "nonce": nonce,
                             "signature": "test_signature_any_length"
                         })
    print(f"    Status: {resp.status_code}")
    data = resp.json()
    ...
    # 7. Проверка после выхода (токен должен быть невалидным)
    print("\n[7] Проверка после выхода...")
    resp = requests.get(f"{BASE_URL}/api/auth/verify",
                       headers={"Authorization": f"Bearer {access_token}"})
    print(f"    Status: {resp.status_code}")
    data = resp.json()
    print(f"    Valid: {data.get('valid', False)} - токен должен быть невалидным")
