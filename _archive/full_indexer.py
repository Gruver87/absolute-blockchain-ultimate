#!/usr/bin/env python3
"""FULL INDEXER - индексирует адреса, газ, receipts"""
...
def update_account(address, timestamp):
    if not address or address == "0x":
        return
    conn = get_conn()
    c = conn.cursor()
    c.execute('''
        INSERT OR IGNORE INTO accounts (address, balance, tx_count, first_seen, last_seen)
        VALUES (?, 0, 0, ?, ?)
    ''', (address, timestamp, timestamp))
    c.execute('''
        UPDATE accounts SET tx_count = tx_count + 1, last_seen = ?
        WHERE address = ?
    ''', (timestamp, address))
    conn.commit()
    conn.close()

def save_block_with_gas(block_data, height):
    conn = get_conn()
    c = conn.cursor()
    ...
    for tx in transactions:
        ...
        gas = 21000  # базовый газ
        
        block_gas += gas
        
        # Обновляем адреса
        update_account(from_addr, timestamp)
        update_account(to_addr, timestamp)
        ...
        c.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (tx_hash, height, from_addr, to_addr, value, "0x1", gas, 1, timestamp))
...
def run():
    print("=" * 60)
    print("🚀 FULL INDEXER (Addresses + Gas + Receipts)")
    print("=" * 60)
    ...
    while True:
        try:
            ...
            if db_h >= node_h:
                print(f"📊 Synced! Height: {node_h} | Accounts: ? | Gas: ?")
                time.sleep(5)
                continue
            ...
            if block_data and isinstance(block_data, dict):
                gas = save_block_with_gas(block_data, height)
                print(f"   ✅ Block {height} saved | Gas: {gas}")
            else:
