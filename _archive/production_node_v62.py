#!/usr/bin/env python3
"""PRODUCTION NODE v62 - с транзакциями, газом и мемпулом"""
...
# Тестовые адреса для транзакций
test_addresses = [
...
class Blockchain:
    ...
    def send_transaction(self, from_addr, to_addr, amount):
        if self.get_balance(from_addr) < amount + GAS_BASE:
    ...
    def generate_test_transaction(self):
        """Генерация тестовой транзакции для оживления сети"""
        from_addr = self.get_wallet()
        to_addr = random.choice(test_addresses)
        amount = random.randint(1, 50)
        return self.send_transaction(from_addr, to_addr, amount)
    
    def mine_block(self, miner=None):
        if not miner:
            miner = self.get_wallet()
        
        # Генерируем тестовые транзакции (1-3 в блок)
        for _ in range(random.randint(1, 3)):
            self.generate_test_transaction()
        
        with mempool_lock:
            txs_to_mine = mempool[:10]
            mempool[:] = mempool[10:]
        ...
        if method == 'eth_blockNumber':
            ...
            if params:
                ...
                success, tx_hash = self.blockchain.send_transaction(
                    tx.get('from'), tx.get('to'), int(tx.get('value', '0x0'), 16))
...
def main():
    print("=" * 60)
    print("PRODUCTION NODE v62 (TRANSACTIONS + GAS)")
    print("=" * 60)
