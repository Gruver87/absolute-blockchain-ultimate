...
try:
    ...
    MEMPOOL_READY = False
    print(f"?? Mempool not available: {e}")
...
class BlockchainDB:
    def __init__(self, db_path: str = "data/blockchain.db"):
        ...
        self._init_database()
        print(f"{C.GREEN}??? Database initialized: {db_path}{C.RESET}")
...
class Consensus:
    def __init__(self):
        ...
        print(f"{C.GREEN}?? Consensus initialized: {self.consensus_type}{C.RESET}")
    ...
    def register_validator(self, address: str, stake: float, commission: float = 5.0) -> str:
        with self.lock:
            ...
            try:
                ...
                print(f"{C.YELLOW}?? DB error: {e}{C.RESET}")
...
class BlockchainCore:
    def __init__(self):
        ...
        print(f"{C.GREEN}??? Blockchain Core: {len(self.chain)} blocks{C.RESET}")
    ...
    def _load_chain(self):
        try:
            ...
            print(f"{C.YELLOW}?? Load chain error: {e}{C.RESET}")
    ...
    def _create_initial_wallets(self):
        ...
        try:
            ...
            print(f"{C.YELLOW}?? Could not create initial wallets: {e}{C.RESET}")
    ...
    def _save_block(self, block: Block):
        try:
            ...
            print(f"{C.YELLOW}?? Save block error: {e}{C.RESET}")
    ...
    def mine_block(self, miner: str) -> Optional[Block]:
        with self.lock:
            ...
            print(f"{C.GREEN}?? Block #{new_block.height} mined by {miner[:16]}... with {len(new_block.transactions)} txs{C.RESET}")
            return new_block
...
class ShardManager:
    def __init__(self):
        ...
        print(f"{C.GREEN}??? Sharding Manager initialized: {self.shard_count} shards{C.RESET}")
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def _get_main_page(self):
        ...
        *{{margin:0;padding:0;box-sizing:border-box}}
        ...
        <a href="/quantum">?? Quantum</a><a href="/sharding">??? Шардинг</a><a href="/plasma">? Plasma</a>
        <a href="/zk">?? ZK</a><a href="/validator">?? Валидаторы</a><a href="/dashboard">?? Дашборд</a>
    ...
    def _get_sharding_page(self):
        return '''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Sharding</title><style>body{font-family:monospace;background:#0f0c29;color:white;padding:20px}</style></head><body><h1>??? Sharding</h1><div id="stats"></div><script>fetch('/api/sharding').then(r=>r.json()).then(d=>{document.getElementById('stats').innerHTML=JSON.stringify(d,null,2)})</script></body></html>'''
    ...
    def _get_validator_page(self):
        return '''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Validators</title><style>body{font-family:monospace;background:#0f0c29;color:white;padding:20px}</style></head><body><h1>?? Validators</h1><div id="validators"></div><script>fetch('/api/validators').then(r=>r.json()).then(d=>{let h='';for(let v of d.validators||[]){h+=`<p>${v.validator_id}: ${v.stake} ABS</p>`}document.getElementById('validators').innerHTML=h})</script></body></html>'''
